...
def businessinsider_author_details(author_link):
    ...
    author_twitter=''
    author_img=''
    author_name = soup.find('h2' , class_='font-bold text-lg md:text-xl text-gray-900').text.strip()
    # print(author_name)
    author_img = "https://www.businessinsider.in"+soup.find('div' ,class_='flex items-center justify-center h-full').find('img',class_ ='rounded w-full h-full')['src']
    # print(author_img)
...
def businessinsider_article_details(url):
    ...
    # author
            ...
            author_link = soup.find('div' ,class_='w-full bi-articleshow').find('a' ,class_= 'artishow_author')
            if author_link:
                author_link = author_link['href']
                # print(author_link)
                author_info = businessinsider_author_details(author_link)
                # print(author_info)
            ...
            auth_container = soup.find('div' ,class_='w-full bi-articleshow').find('h2' ,class_= 'font-bold text-[14px] md:text-[16px] text-blue-600')
            # artishow_author
            if auth_container:
...
if __name__ == "__main__":
    ...
    with open("businessinsider_articles.json", "w") as json_file:
